...
export default function App() {
  ...
  // Trading ticket state with dual-mode sizing (Shares vs Dollars)
  const [orderType, setOrderType] = useState('BUY');
  const [orderShares, setOrderShares] = useState('');
  const [inputMode, setInputMode] = useState('shares'); // 'shares' | 'dollars'
  ...
  const handleSharesChange = (val) => {
    ...
    if (!isNaN(num) && num > 0 && quote?.price) {
      setTargetDollarAmount((num * quote.price).toFixed(2));
    } else {
      setTargetDollarAmount('');
    }
  };

  const handleDollarAmountChange = (val) => {
    setTargetDollarAmount(val);
    const num = parseFloat(val);
    if (!isNaN(num) && num > 0 && quote?.price) {
      // Snap to nearest whole share quantity and fit dollar price
  ...
  const applyCashPercentage = (pct) => {
    if (!activePortfolio?.cashBalance || !quote?.price) return;
    const target = activePortfolio.cashBalance * (pct / 100);
    handleDollarAmountChange(target.toFixed(2));
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full">
        ...
        {activeTab === 'markets' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Execution Order Ticket with Dual Sizing (Shares vs Dollars) */}
            <div className="space-y-6">
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5 font-sans">
                      <label className="text-[11px] uppercase tracking-wider text-slate-600">
                        {inputMode === 'shares' ? 'Share Quantity' : 'Dollar Investment ($)'}
                      </label>
                      <div className="flex bg-slate-100 p-0.5 rounded text-[10px] font-mono">
                        ...
                        <button
                          ...
                          onClick={() => {
                            setInputMode('dollars');
                            if (orderShares && quote?.price) {
                              setTargetDollarAmount((parseFloat(orderShares) * quote.price).toFixed(2));
                            }
                          }}
                          className={`px-2 py-0.5 rounded transition ${inputMode === 'dollars' ? 'bg-white font-bold text-slate-950 shadow-xs' : 'text-slate-500'}`}
                        >
                          Dollars ($)
                        </button>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pl-7 pr-3 py-2.5 bg-white border border-slate-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-950 font-mono text-sm"
                          />
                        </div>
                        ...
                        {parseFloat(targetDollarAmount) > 0 && quote?.price && (
                          <div className="p-2 bg-blue-50 border border-blue-200 rounded-lg text-[11px] font-mono text-blue-900 space-y-0.5">
                            <div className="flex justify-between">
                              <span>Nearest Shares:</span>
                              <span className="font-bold">{orderShares} shares</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Fitted Total:</span>
                              <span className="font-bold">${((parseFloat(orderShares) || 0) * quote.price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                            </div>
                          </div>
                        )}
                      </div>
